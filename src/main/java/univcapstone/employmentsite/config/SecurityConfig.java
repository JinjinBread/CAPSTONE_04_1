...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        return httpSecurity.httpBasic(AbstractHttpConfigurer::disable)
                ...
                .cors(Customizer.withDefaults()) //HTTP 기본 인증 및 CSRF 보안 비활성화, CORS 활성화
                ...
                .authorizeHttpRequests(authorizeRequests ->
                        authorizeRequests //요청 인증 무시 목록
                                .requestMatchers("/", "/join", "/joincheck", "/login/**",
                                        "/logout", "/confirm/**", "/verify/**",
                                        "/find/**", "/favicon.ico").permitAll()
                                //회원가입, 개인정보 동의, 로그인, 로그아웃, 이메일 인증, 아이디 중복 확인, 아이디 및 비밀번호 찾기, 파비콘
                                .anyRequest().authenticated()) //위 URI 외의 URI는 모두 인증 필수
